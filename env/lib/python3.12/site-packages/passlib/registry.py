...
def register_crypt_handler_path(name, path):
    ...
    for a class named ``myhash`` within the ``kpg_crm.helpers`` module::

        >>> from passlib.registry import registry_crypt_handler_path
        >>> registry_crypt_handler_path("myhash", "kpg_crm.helpers")

    ...while this form would cause ``get_handler("myhash")`` to look
    for a class name ``MyHash`` within the ``kpg_crm.helpers`` module::

        >>> from passlib.registry import registry_crypt_handler_path
        >>> registry_crypt_handler_path("myhash", "kpg_crm.helpers:MyHash")
    """
    # validate name
